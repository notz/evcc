...
func TestContinuous_CheapestContiguousSlots(t *testing.T) {
	...
	plan := p.Plan(2*time.Hour, 0, now.Add(6*time.Hour), true, nil)
...
func TestContinuous_WindowWithPastRates(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, 0, targetTime, true, nil)
...
func TestContinuous_WindowAllRatesInPast(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, 0, targetTime, true, nil) // continuous mode
...
func TestContinuous_TargetAfterKnownPrices(t *testing.T) {
	...
	plan := p.Plan(40*time.Minute, 0, clock.Now().Add(2*time.Hour), true, nil) // charge efficiency does not allow to test with 1h
	assert.False(t, !SlotAt(clock.Now(), plan).IsZero(), "should not start if car can be charged completely after known prices ")

	plan = p.Plan(2*time.Hour, 0, clock.Now().Add(2*time.Hour), true, nil)
...
func TestContinuous_Precondition(t *testing.T) {
	...
	plan := p.Plan(tariff.SlotDuration, tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), true, nil)
	...
	plan = p.Plan(2*tariff.SlotDuration, tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), true, nil)
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now(),
	...
	plan = p.Plan(time.Duration(1.5*float64(tariff.SlotDuration)), tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), true, nil)
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now(),
	...
	plan = p.Plan(tariff.SlotDuration, 24*time.Hour, clock.Now().Add(time.Hour), true, nil)
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now().Add(3 * tariff.SlotDuration),
			End:   clock.Now().Add(4 * tariff.SlotDuration),
...
func TestContinuous_Precondition_NonSlotBoundary(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, precondition, targetTime, true, nil)
...
func TestPrecondition_Everything(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, precondition, targetTime, false, nil)
	...
	planContinuous := p.Plan(requiredDuration, precondition, targetTime, true, nil)
...
func TestContinuous_ContinuousPlanNoTariff(t *testing.T) {
	...
	plan := p.Plan(time.Hour, 0, clock.Now(), true, nil)
...
func TestContinuous_ContinuousPlan(t *testing.T) {
	...
	plan := p.Plan(150*time.Minute, 0, clock.Now(), true, nil)
...
func TestContinuous_ContinuousPlanOutsideRates(t *testing.T) {
	...
	plan := p.Plan(30*time.Minute, 0, clock.Now(), true, nil)
...
func TestContinuous_StartBeforeRates(t *testing.T) {
	...
	plan := planner.Plan(requiredDuration, 0, targetTime, true, nil) // continuous mode
...
func TestContinuous_ExcessTimeFinishesAtTarget(t *testing.T) {
	...
	plan := planner.Plan(requiredDuration, 0, targetTime, true, nil) // continuous, no precondition
	...
	plan = planner.Plan(requiredDurationShort, 0, targetTime, true, nil) // continuous, no precondition
	...
	plan = planner.Plan(requiredDurationMedium, 0, targetTime, true, nil) // continuous, no precondition
