...
func TestNilTariff(t *testing.T) {
	...
	plan := p.Plan(time.Hour, 0, clock.Now().Add(30*time.Minute), false, nil)
...
func TestFlatTariffLongSlots(t *testing.T) {
	...
	plan := p.Plan(time.Hour, 0, clock.Now().Add(2*time.Hour), false, nil)
	...
	// expect 00:00-01:00 UTC
	plan = p.Plan(time.Hour, 0, clock.Now().Add(time.Hour), false, nil)
	assert.Equal(t, api.Rate{Start: clock.Now(), End: clock.Now().Add(time.Hour)}, SlotAt(clock.Now(), plan))
}
...
func TestTargetAfterKnownPrices(t *testing.T) {
	...
	plan := p.Plan(40*time.Minute, 0, clock.Now().Add(2*time.Hour), false, nil) // charge efficiency does not allow to test with 1h
	assert.False(t, !SlotAt(clock.Now(), plan).IsZero(), "should not start if car can be charged completely after known prices ")

	plan = p.Plan(2*time.Hour, 0, clock.Now().Add(2*time.Hour), false, nil)
...
func TestChargeAfterTargetTime(t *testing.T) {
	...
	plan := p.Plan(time.Hour, 0, clock.Now(), false, nil)
	assert.Equal(t, simplePlan, plan, "expected simple plan")
...
func TestPrecondition(t *testing.T) {
	...
	plan := p.Plan(tariff.SlotDuration, tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), false, nil)
	assert.Equal(t, api.Rates{
		{
	...
	plan = p.Plan(2*tariff.SlotDuration, tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), false, nil)
	...
	plan = p.Plan(time.Duration(1.5*float64(tariff.SlotDuration)), tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), false, nil)
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now(),
	...
	plan = p.Plan(tariff.SlotDuration, 24*time.Hour, clock.Now().Add(time.Hour), false, nil)
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now().Add(3 * tariff.SlotDuration),
			End:   clock.Now().Add(4 * tariff.SlotDuration),
...
func TestContinuousPlan(t *testing.T) {
	...
	plan := p.Plan(150*time.Minute, 0, clock.Now(), false, nil)
...
func TestContinuousPlanOutsideRates(t *testing.T) {
	...
	plan := p.Plan(30*time.Minute, 0, clock.Now(), false, nil)
...
func TestStartBeforeRatesInsufficientTime(t *testing.T) {
	...
	plan := planner.Plan(requiredDuration, 0, targetTime, false, nil) // dispersed mode
...
func TestEmptyRatesAfterClamping(t *testing.T) {
	...
	plan := p.Plan(time.Hour, 0, c.Now().Add(90*time.Minute), false, nil)
